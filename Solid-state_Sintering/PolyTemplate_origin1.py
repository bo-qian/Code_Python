...
primary_directory = os.getcwd() + "/TGG_Simulation_origin_version"
...
# 创建Readme文件
readme_file_path = primary_directory + '/Readme.txt'
...
def particle_centers_coord(template_particle_center, template_particle_dimensions, radius_particle, particle_layer,
    ...
    for k in range(particle_layer):
        ...
        for m in range(layer[k][0]):
            ...
            y_coordinate = float(
                template_particle_center[1] + 0.5 * template_particle_dimensions[1] + (1 + k * sqrt(3)) *
...
def particle_centers_coordinate(layer_number, aspect_ratio_template, template_particle, radius_particle, particle_number_total):
    particle_radius = [radius_particle] * particle_number_total
    particle_centers_radius = []
    for i in range(layer_number):
        for j in range(aspect_ratio_template + layer_number + 1 + i):
            x_coordinate = float(
                template_particle.center[0] + (j - (aspect_ratio_template + layer_number + i) / 2) * radius_particle * 2)
...
particle_centers, particle_radii = particle_centers_coordinate(layer_numbers, aspect_ratio, template_particle, radius_particle, particle_number_total)
...
# 定义斯托克斯方程弱形式
stokes_term1 = inner(pde.sigma(v_trial, c_new), grad(v_test)) * dx
...
stress_R_term1 = pde.sigma(v, c_new) * p * Identity(2)
