...
primary_directory = os.getcwd() + "/PolyParticle_Simulation"
...
filename = 'PPSS-' + str(num_subdirectories + 1) + ' (' + datetime.datetime.now().strftime("%B %d, %Y, %H-%M-%S") + ")"
...
logging.info(f"The smoothness coefficient epsilon is {epsilon}")
...
xdmf_file = XDMFFile(Output_directory + f"/solution.xdmf")
xdmf_file.parameters["flush_output"] = True
xdmf_file.parameters["functions_share_mesh"] = True

try:
    # 重置初始条件 ************************************************************************************************
    print(f"\nTime step: {counterStep}, Time: {timeCurrent}, Time of Simulation: {Time_Simulation}")
    assign(u_new, [c0, mu0])
    u_prev.assign(u_new)
    ...
    name_unit = np.array(
        [['Time', 'Surface Energy', 'Grain Boundary Energy', 'Total Free Energy', 'Area of Grain Boundary', 'Normalized '
                                                                                                            'Time',
    ...
    print(f"the total free energy at time {timeCurrent} is {data_curve[counterStep, 3]:.4e}J\n"
          f"the surface energy at time {timeCurrent} is {data_curve[counterStep, 1]:.4e}J\n"
          f"the grain boundary energy at time {timeCurrent} is {data_curve[counterStep, 2]:.4e}J\n"
          f"the area of grain boundary at time {timeCurrent} is {data_curve[counterStep, 4]:.4e}m^2\n"
          f"the surface energy at time {timeCurrent} is {data_curve[counterStep, 6]:.4f}\n"
          f"the grain boundary energy at time {timeCurrent} is {data_curve[counterStep, 7]:.4f}\n"
          f"the total free energy at time {timeCurrent} is {data_curve[counterStep, 8]:.4f}"
          )
    ...
    while timeCurrent < Time_Simulation:
        ...
        if np.mod(counterStep, frequencyOutput) == 0:
            ...
            plot_curve(data_curve[:counterStep + 1, 0], data_curve[:counterStep + 1, 1], "J", "Surface Energy",
                       timeCurrent)
            plot_curve(data_curve[:counterStep + 1, 0], data_curve[:counterStep + 1, 2], "J", "Grain Boundary Energy",
                       timeCurrent)
            plot_curve(data_curve[:counterStep + 1, 0], data_curve[:counterStep + 1, 3], "J", "Total Free Energy",
                       timeCurrent)
            plot_curve(data_curve[:counterStep + 1, 0], data_curve[:counterStep + 1, 4], r"$\mathrm{m^2}$",
