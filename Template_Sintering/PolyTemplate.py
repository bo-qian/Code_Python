...
primary_directory = os.getcwd() + "/TGG_Simulation"
...
filename = 'TGGSS-' + str(num_subdirectories + 1) + ' (' + datetime.datetime.now().strftime("%B %d, %Y, %H-%M-%S") + ")"
...
# Read the input file ****************************************************************************************
parameters_input = read_input_file("input_template.txt")
...
shutil.copy("input_template.txt", os.path.join(Input_directory, "input_template.txt"))
...
try:
    ...
    name_unit = np.array(
        [['Time', 'Surface Energy', 'Grain Boundary Energy', 'Total Free Energy', 'Area of Grain Boundary', 'Normalized Surface Energy', 'Normalized Grain Boundary Energy', 'Normalized Total Free Energy'],
         ['s', 'J', 'J', 'J', 'm^2', ' ', ' ', ' ']])
