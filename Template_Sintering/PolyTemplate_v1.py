...
# Date: 2024-06-06
# 1st update: 2024-06-24
# 2nd update: 2024-07-01
# 3rd update: 2024-07-14
# ************************************************************************************************************
import logging
import csv
import sys
...
class SolidStateSintering:
    ...
    def sigma(self, u, C, eta):
        ...
        return ((self.viscosity_ratio + (1 - self.viscosity_ratio) * self.N(C)) * self.muBulk + self.muSurf * (
                C ** 2) * ((1 - C) ** 2) + 2 * self.muGB * self.N(C) * sum(
            [eta[i] ** 2 * (sum([eta[j] ** 2 for j in range(self.Np)]) - eta[i] ** 2) for i in range(self.Np)])) * (
                grad(u) + grad(u).T)
...
def initial_schematic(coordinates, radii, domain, ratio):
    ...
    with plt.rc_context(
        ...
        rect = patches.Rectangle((domain[0]/2-ratio*radii[0], domain[1]/2-radii[0]), ratio*radii[0]*2, radii[0]*2, edgecolor='black',fill=True, facecolor='white', linewidth=3, zorder=2)
        plt.text(domain[0]/2, domain[1]/2, rf"$\eta_{{{len(coordinates) + 1}}}$", fontsize=32, ha='center', va='center', zorder=3)
        ax.add_patch(rect)
        ...
        plt.title('Template Sintering', pad=20, fontweight='bold')
...
# Read the input file ****************************************************************************************
parameters_input = read_input_file("input_template_v1.txt")
...
primary_directory = os.getcwd() + "/TGG_Simulation_v1"
...
if timeInitial == 0:
    filename = 'TGGSS-V1-' + str(num_subdirectories + 1) + ' (' + datetime.datetime.now().strftime(
        "%B %d, %Y, %H-%M-%S") + ")"
else:
...
shutil.copy("input_template_v1.txt", os.path.join(Input_directory, "input_template_v1.txt"))
...
domain_size = [float((aspect_ratio + layer_numbers * 2 + 2) * particle_size),
               float((1 + layer_numbers * 2 + 2) * particle_size)]
...
# Set the epsilon of the template grain
template_particle.set_epsilon(1)
# Calculate the area of the template grain
...
data_curve = np.empty((NumberOfTimeStep + 1, 8))
...
try:
    if timeInitial == 0:
        # Output the input parameters to the log file *****************************************************************
        logging.info("======================================")
        ...
        logging.info(f"The aspect ratio of template is {aspect_ratio}")
        logging.info(f"The layer number of particles is {layer_numbers}")
        # Algorithmic parameters
        logging.info("#################################################")
        ...
        # Calculate the volume fraction of the template grain
        volume_fraction_template = aspect_ratio / (aspect_ratio + particle_number_total * np.pi / 4)
        logging.info(f"The volume fraction of template grain: {volume_fraction_template * 100:.2f}%")
        logging.info("#################################################")
        ...
        print(f"\nTime step: {counterStep}, Time: {timeCurrent:}s, Time of Simulation: {Time_Simulation:}s")
        assign(u_new, [c0, mu0])
        u_prev.assign(u_new)
        ...
        print("----- Solving Stokes equation in initial time step -----")
        assign(v_combined,
        ...
        data_curve[counterStep, 5] = assemble(surface_energy)
        data_curve[counterStep, 6] = assemble(grain_boundary_energy)
        data_curve[counterStep, 7] = assemble(E)

        with open(data_curve_path, mode='a', newline='') as csv_file:
            writer = csv.writer(csv_file)
            writer.writerow([timeCurrent, data_curve[counterStep, 1], data_curve[counterStep, 2], data_curve[counterStep, 3],
                             data_curve[counterStep, 4], data_curve[counterStep, 5], data_curve[counterStep, 6],
                             data_curve[counterStep, 7]])
        ...
        print(f"the total free energy at time {timeCurrent:g}s is {data_curve[counterStep, 3]:.4e}J\n"
              ...
              f"the surface energy at time {timeCurrent:g}s is {data_curve[counterStep, 5]:.4f}\n"
              f"the grain boundary energy at time {timeCurrent:g}s is {data_curve[counterStep, 6]:.4f}\n"
              f"the total free energy at time {timeCurrent:g}s is {data_curve[counterStep, 7]:.4f}"
              )

        print("----- Plotting figures of C, eta, eta_pure, v and stress -----")
        initial_schematic(particle_centers, particle_radii, domain_size, aspect_ratio)
        ...
        with open(readme_file_path, 'a', encoding='utf-8') as file:
            file.write(
                ...
                f"Output frequency : once every {frequencyOutput} time steps\n"
                f"The aspect ratio of template : {aspect_ratio}\n"
                f"Number of particle layer: {layer_numbers}\n"
    ...
    while timeCurrent < Time_Simulation:
        ...
        print(f"the total free data_curve at time {timeCurrent:g}s is {data_curve[counterStep, 3]:.4e}J\n"
              f"the surface data_curve at time {timeCurrent:g}s is {data_curve[counterStep, 1]:.4e}J\n"
              f"the grain boundary data_curve at time {timeCurrent:g}s is {data_curve[counterStep, 2]:.4e}J\n"
              f"the area of grain boundary at time {timeCurrent:g}s is {data_curve[counterStep, 4]:.4e}m^2\n"
              f"the surface energy at time {timeCurrent:g}s is {data_curve[counterStep, 5]:.4f}\n"
              f"the grain boundary energy at time {timeCurrent:g}s is {data_curve[counterStep, 6]:.4f}\n"
              f"the total free energy at time {timeCurrent:g}s is {data_curve[counterStep, 7]:.4f}"
        ...
        if np.mod(counterStep, frequencyOutput) == 0:
            ...
            if os.path.exists(Output_directory + f"/checkpoint_{counterStep - 1 - frequencyOutput}.h5") and counterStep != counterStepInitial:
                os.remove(Output_directory + f"/checkpoint_{counterStep - 1 - frequencyOutput}.h5")
            ...
            plot_curve(data_curve[:counterStep + 1, 0], data_curve[:counterStep + 1, 1], "Surface Energy", "Time (s)", "Surface Energy (J)", timeCurrent)
            plot_curve(data_curve[:counterStep + 1, 0], data_curve[:counterStep + 1, 2], "Grain Boundary Energy", "Time (s)", "Grain Boundary Energy (J)", timeCurrent)
            plot_curve(data_curve[:counterStep + 1, 0], data_curve[:counterStep + 1, 3], "Total Free Energy", "Time (s)", "Total Free Energy (J)", timeCurrent)
            plot_curve(data_curve[:counterStep + 1, 0], data_curve[:counterStep + 1, 4], "Area of Grain Boundary", "Time (s)", r"Area of Grain Boundary ($\mathrm{m^2}$)", timeCurrent)
            plot_curve(data_curve[:counterStep + 1, 0], data_curve[:counterStep + 1, 5], "Normalized Surface Energy", "Time (s)", "Normalized Surface Energy", timeCurrent)
            plot_curve(data_curve[:counterStep + 1, 0], data_curve[:counterStep + 1, 6], "Normalized Grain Boundary Energy", "Time (s)", "Normalized Grain Boundary Energy", timeCurrent)
            plot_curve(data_curve[:counterStep + 1, 0], data_curve[:counterStep + 1, 7], "Normalized Total Free Energy", "Time (s)", "Normalized Total Free Energy", timeCurrent)
            print("Plot finished.")
