...
primary_directory = os.getcwd() + "/PolyParticle_Viscosity_Simulation_GBDF2_V1"
...
# Read the input file
parameters_input = read_input_file("input_polyparticle_viscosity_BDF2_V1.txt")
shutil.copy("input_polyparticle_viscosity_BDF2_V1.txt", os.path.join(Input_directory, "input_polyparticle_viscosity_BDF2_V1.txt"))
...
try:
    ...
    # The linear weak form of c  under gbdf2  c_mid = (1.0 - theta) * c_prev + theta * c_trial#半隐式是否去除
    gbdf2_c_WeakForm = ((2 * beta + 1) * c_trial - 4 * beta * c_prev + (2 * beta - 1) * c_forward) * c_test * dx + dt * dot(pde.M(E2_prev(c_prev, c_forward)) * grad(E1_trail(mu_trial, mu_prev)), grad(c_test)) * dx - dt * dot(E2_prev(v_prev, v_forward) * E1_trail(c_trial, c_prev), grad(c_test)) * dx
    # gbdf2_mu_WeakForm = E1_trail(mu_trial,mu_prev) * mu_test * dx  \
    # - pde.kc * dot(grad((1.0 - theta) * E2_prev(c_prev,c_forward)+ theta * E1_trail(c_trial,c_prev)), grad(mu_test)) * dx \
     #- (pde.fmu(E2_prev(c_prev,c_forward)) + pde.c_stabilizer * (c_trial - c_prev)) * mu_test * dx

    gbdf2_mu_WeakForm = E1_trail(mu_trial,mu_prev) * mu_test * dx  \
    - pde.kc * dot(grad( E1_trail(c_trial,c_prev)), grad(mu_test)) * dx \
    - (pde.fmu(E2_prev(c_prev,c_forward)) + pde.c_stabilizer * (c_trial - c_prev)) * mu_test * dx
    ...
    while timeCurrent < timeSimulation:
        ...
        if np.mod(counterStep, frequencyOutput) == 0:
            ...
            plot_curve(various_data[:counterStep + 1, 4], various_data[:counterStep + 1, 5], "Normalized Shrinkage", "Normalized Time",
