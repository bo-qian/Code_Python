...
# Wrote by Daixiaoxu at Shanghai University
# Date: 2024-06-23
...
primary_directory = os.getcwd() + "/PolyParticle_Viscosity_Simulation_testing"
...
filename = 'PPVS-' + str(num_subdirectories + 1) + ' (' + datetime.datetime.now().strftime("%B %d, %Y, %H-%M-%S") + ")"
...
with open(readme_file_path, 'w', encoding='utf-8') as file:
    file.write(f"####################################################################\n"
               ...
               f"Operated by Daixiaoxu at Shanghai University\n"
               f"Date: {time_stamp}\n"
               f"####################################################################\n\n")
...
# Read the input file
parameters_input = read_input_file("input_polyparticle_viscosity.txt")
shutil.copy("input_polyparticle_viscosity.txt", os.path.join(Input_directory, "input_polyparticle_viscosity.txt"))
...
try:
    ...
    # calculate the growth rate of neck
